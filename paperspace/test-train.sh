...
#    recurrent_dropout_rate_opt="-r 0.0"
#else
...
echo "python train/train.py -i /storage -o /artifacts -b 128 -c 16 -e 1 -l DEBUG  test"

gradient experiments run singlenode \
    --name test-${bidirectional_name}LSTM${balance_name}${lstm_cells}-dr${dropout_rate}-rdr${recurrent_dropout_rate}-batch${batch_size}-lr${learning_rate}-${sample_size} \
    --projectId pr1cl53bg \
    --machineType ${machine_type} \
    --container vtluk/paperspace-tf-gpu:1.0 \
    --command "python train/train.py -i /storage -o /artifacts -b 128 -c 16 -e 1 -l DEBUG ${sample_size}" \
    --workspace . \
